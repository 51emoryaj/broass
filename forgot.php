...
if(isset($_POST['submit'])){
    ...
    if($usertype == 'Administrator'){
        $query = $conn->query("SELECT * FROM `tbl_admin` WHERE `username`='$email';");
    }
    elseif($usertype == 'Staff'){
        $query = $conn->query("SELECT * FROM `tbl_staff` WHERE `username`='$email';");
    }
    else {
        die("<script>alert('Invalid Usertype!');window.location='forgot.php';</script>");
    }
    if($query->num_rows > 0){
        $token = md5(base64_encode(md5($email.uniqid())));
        $url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') ? 'https://' : 'http://'. $_SERVER['SERVER_NAME'] . dirname($_SERVER['PHP_SELF']).'/reset.php?resetpass&token='.$token.'&email='.$email.'&usertype='.$usertype;
        $body = "
        Dear User,<br><br>
        ...
        if(!$mail->Send()){
            ...
            $query = $conn->query("INSERT INTO `tbl_resetpass` VALUES (NULL, '$email', '$token', '$usertype')");
            if($query){
                die("<script>alert('Please check your email inbox or spam folder to reset your password.');window.location='index.php';</script>");
            } else {
                die("Error: ".$conn->error);
            }
        }
    } else {
        die("<script>alert('There\'s no user registered with that email!');window.location='forgot.php';</script>");
    }
...
<html>
  <head>
    ...
    <title>Forgot Password - BROASS</title>
          ...
          <div class="card shadow-lg o-hidden border-0 my-5">
            <div class="card-body">
              <div class="p-5">
                ...
                <div class="mb-3">
                    <select class="form-select form-control-user" name="usertype" id="usertype" required>
                ...
                <div class="mb-3 user d-flex justify-content-center">
                    Already remembered?&nbsp;<a href="index.php" style="text-decoration:none;">Login here.</a>
                </div>
              </div>
            </div>
          </div>
