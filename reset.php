...
if(isset($_GET['resetpass'])){
  ...
  $query = $conn->query("SELECT * FROM `tbl_resetpass` WHERE `email`='$email' AND `token`='$token' AND `usertype`='$usertype'");
  if($query->num_rows > 0){
    if(isset($_POST['submit'])){
        $password = md5($_POST['password']);
        if($usertype == 'Administrator'){
          $query = $conn->query("UPDATE `tbl_admin` SET `password`='$password' WHERE `username`='$email';");
        }
        elseif($usertype == 'Staff'){
          $query = $conn->query("UPDATE `tbl_staff` SET `password`='$password' WHERE `username`='$email';");
        }
        if($query){
            $query = $conn->query("DELETE FROM `tbl_resetpass` WHERE `token`='$token'");
            if($query){
                die("<script>alert('Your password has been successfully reset! You can now log in with your new password!');window.location='index.php';</script>");
            } else {
...
<html>
  <head>
    ...
    <title>Reset Password - BROASS</title>
